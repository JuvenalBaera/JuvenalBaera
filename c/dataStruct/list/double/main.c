...
NODE* removeData(LIST *list, int id){
    NODE *aux, *rem = NULL;
    if(list->begin){
        if(list->begin->data.id == id){ // If is First data to remove
            rem = list->begin;
            list->begin = rem->next;
            if(list->begin)
...
void showListBack(NODE *node){
    printf("========= LIST =========\n");
    while(node){
        printf("\t");
        showPerson(node->data);
        node = node->prev;
    }
    printf("=======================\n");
}
...
void mainMenu(int *option){
    ...
    printf("\t7 - Show Queue\n");
    printf("\t8 - Show Queue Backward\n\n");
    *option = readInt(0, 8, "Choose one option[0 - 8]: ");
    putchar('\n');
}
...
int main(){
    ...
    do{
        ...
        switch(option){
            ...
            case 8: showListBack(last(&list)); break;
            default:
